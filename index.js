...
const username = encodeURIComponent("osnaren");
const password = encodeURIComponent("66@Dbmongo123");

const { MongoClient, ServerApiVersion } = require('mongodb');
const uri = `mongodb+srv://${username}:${password}@cluster0.xtgtr.mongodb.net/touring?retryWrites=true&w=majority`;
...
// Serve Users Json
var url = "mongodb+srv://osnaren:66@Dbmongo123@cluster0.xtgtr.mongodb.net/touring?retryWrites=true&w=majority";
app.get('/json', function (req, res) {
    var queryres = null;
    mclient.connect(function (err, db) {
        if (err) throw err;
        var dbo = db.db("touring");
        dbo.collection("trips").find({}, {projection: {user: 1}}).toArray(function (err, result) {
            if (err) throw err;
            console.log(result);
            queryres = result;
            db.close().then(r => {
...
app.get('/currentuser', function (req, res) {
    ...
    mclient.connect( function (err, db) {
        if (err) throw err;
        var dbo = db.db("touring");
        dbo.collection("trips").find({current: "true"}).toArray(function (err, result) {
            if (err) throw err;
            console.log(result);
            queryres = result;
            db.close().then(r => {
    ...
    mclient.connect( function (err, db) {
        ...
        dbo.collection("trips").find({current: "true"}).toArray(function (err, result) {
            ...
            db.close().then(r => {
                ...
                client.verify.services.create({friendlyName: 'HodoPhilics'})
                    .then(service => {
    ...
    client.verify.services(vid)
        ...
        if (status === 'approved') {
            mclient.connect( function (err, db) {
                ...
                dbo.collection("trips").updateOne(myquery, newvalues, function (err, res) {
                    if (err) throw err;
...
app.post('/login', function (req, res) {
    ...
    mclient.connect( function (err, db) {
        if (err) throw err;
        var dbo = db.db("touring");
        dbo.collection("trips").find({user: user}, {projection: {password: 1}}).toArray(async function (err, result) {
            if (err) {
                res.send(false);
                throw err;
            }
...
app.get('/logout', function (req, res) {
    ...
    mclient.connect( function (err, db) {
        ...
        dbo.collection("trips").find(myquery, {projection: {user: 1}}).toArray(async function (err, result) {
            if (err) {
                res.send(false);
                throw err;
            }
...
app.post('/signup', async function (req, res) {
    ...
    mclient.connect( function (err, db) {
        if (err) throw err;
        var dbo = db.db("touring");
        dbo.collection("trips").insertOne(redt, function (err, res) {
            if (err) throw err;
